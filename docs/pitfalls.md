...
20. **文档列表 `PrintJob/Get` 是最终一致的，滞后能到十几分钟 —— 不能拿它做同步确认。**

    上传接口返回 `code=0` 之后，曾经会去文档列表里找同名的记录，找不到就报
    "已提交，但未收到系统确认"（这个兜底是当年进度通道还在的时候加的）。实测下来
    这个判断是错的：

    ```
    10:16:39  作业上传（服务端 szFileName 里的时间戳 = 20260918181639）
    10:17:30  上传请求终于返回 code=0
    10:17:30  开始查文档列表，12 秒窗口，一次都没命中 → 报"提交失败"
    ~10:31    再查，文档在列表里，szJobName = "云打印-20260918-181639.pdf"
              和提交时用的名字**逐字一致**，dwJobId = 6691251
    ```

    名单接口本身单次就要 4 秒左右（`rawFetch` 的超时是 20 秒），12 秒只够采样
    两三次；而且入库比上传返回还慢。把窗口放宽到 45 秒仍然命不中 —— 因为这不是
    "慢一点"，是**最终一致**，十几分钟后才稳定可见。

    两个后果都很糟：
      - 明明成功的作业被报成失败，用户重打一份 → 队列里两份；
      - 失败响应带着 `retryable: true`，Web 上传界面据此**自动重传**最多 N 次
        → 一次点击在队列里留下好几份。

    现在改成：上传接口的 `code=0` + 2xx 就是成功，不再做二次确认。
    "到底进没进队列"由用户在「文档」页面自己看 —— 那个页面本来就是干这个的。
    进度通道（WebSocket）如果哪天恢复，它报的 `error` 仍然是有效的失败信号，
    所以那段代码留着，但它同样不再用来"兜底核实"。

21. **上游对"不认识的取值"不报错，只按默认处理 —— 参数写错在本地一点都看不出来。**

    驱动侧原来的 `POST /api/driver/print` 自己发明了一套 0 起的编号：
    `x-color` 夹在 `0..1`、缺省 0，`x-duplex` 夹在 `0..2`、缺省 0。看起来没毛病，
    实际后果是**从系统打印对话框打印的作业永远是黑白**：`dwColor=0` 上游不认识，
    它不报错、直接按默认的黑白入库。上传照样成功、队列里照样有这份文件，
    所以本地怎么测都是"好的"。
    ...
    通用教训：**跨系统传枚举值，不要自己发明一套"等价"的编号**，去把对方的
    表单/协议原文抠出来照抄；而且要有一条"逐字段核对真实请求"的测试
    （见 `tools/test-driver-options.mjs`）—— 否则这类 bug 只有用户能发现。

    同一类问题在**读**的方向上也出现过：队列返回的 `szAttribe` 词表我们本来是
    照着一个自己写的 mock 猜的（`single,nocolor` / `double,color`），真账号实测
    完全不是那回事：

    ```
    传 dwDuplex=1 + dwColor=1（黑白）  →  szAttribe = "single,"
    传 dwDuplex=2 + dwColor=1（黑白）  →  szAttribe = "vdup,"
    传 dwDuplex=3 + dwColor=2（彩色）  →  szAttribe = "hdup,color,"
    ```

    双面是 `hdup`（长边）/`vdup`（短边）而不是 `double`；**黑白根本没有 token**，
    只有彩色才带 `color`。所以"`includes("color")` 之外一律算黑白"是错的判断方式，
    正确写法是：切成标签 → 认标签 → 颜色取"有 `color` 就是彩色，有其它标签但没
    `color` 才是黑白"。（顺便：当年猜出来的 `nocolor` 里也含 `color` 子串，
    于是 `includes("color")` 把每一份黑白作业都显示成"彩色"。）

23. **`dwDuplex` 的 2 是短边、3 是长边 —— 和"2 在前所以是长边"的直觉相反。**

    官方表单里 `value=2` 标着"双面短边"、`value=3` 标着"双面长边"。我们按常识
    写反过一次（界面标签 2=长边 3=短边），后果是装订方向正好错开：想打成一本书，
    出来的是便签本方向。值得记一笔的是这个错**能用两条独立证据交叉验证**：
    官方表单的标签，以及真账号上传后队列返回的 `dwDuplex=3 → hdup`（长边）。
    这类"编号↔语义"的映射，值是抄来的，就别再按语义重排。

24. **"份数"这一项，CUPS 只调用一次 backend，`num-copies` 是参数。**

    曾经担心"把份数转给云端会重复出纸"（如果 cupsd 对 `-n 3` 调用三次 backend，
    每一次都带 `copies=3`，云队列里就会出现 9 份）。实测确认不会：建一个临时
    socket 队列 `lp -n 3`，本机 TCP 只收到 **1 次**连接。

    原因是我们的队列是 PDF 直通，滤镜链里没有 `pstops` —— 复制份数本来是
    `pstops`（PostScript）或设备自己做的事。PPD 里 `*cupsManualCopies: False`
    表示"设备支持自己出多份"，所以 backend 拿到 `argv[4]=3` 就该原样转达。

    通用教训：**"我改的这一处会不会和系统已有的行为叠加"要用一次实验回答**，
    别靠推理 —— 这种叠加错误（3×3=9 份）恰恰是本地小样本测不出来的。
